import 'dart:math';
import 'package:dkatalis/model/user.dart';
...
class CardStack extends StatefulWidget {
  ...
  final UserFetchService matchEngine;

  final Widget rangetext;
  final bool overlayOrNot;
  CardStack(
      {this.matchEngine,
      this.onSwipeCallback,
      this.rangetext,
      this.overlayOrNot = true});
...
class _CardStackState extends State<CardStack> {
  ...
  void initState() {
    ...
    _currentMatch = widget.matchEngine.currentUser;
    _currentMatch.addListener(_onMatchChange);

    _frontCard = new Key(_currentMatch.user?.name?.name ?? '');
  }
  ...
  Widget _buildBackCard() {
    return Transform(
      ...
      child: ProfileCard(
        user: widget.matchEngine.nextUser.user,
      ),
    );
  }
  ...
  Widget _buildFrontCard() {
    return ProfileCard(
      key: _frontCard,
      user: widget.matchEngine.currentUser.user,
    );
  }

  SlideDirection _desiredSlideOutDirection() {
    switch (widget.matchEngine.currentUser.decision) {
      case Decision.nope:
  ...
  void _onSlideComplete(SlideDirection direction) {
    FetchService currenMatch = widget.matchEngine.currentUser;

    switch (direction) {
      case SlideDirection.left:
        currenMatch.left();
        if (widget.onSwipeCallback != null)
    ...
    widget.matchEngine.cycleUser();
  }

  @override
  Widget build(BuildContext context) {
    if (!widget.overlayOrNot) return Offstage();
    return Stack(
      children: <Widget>[
        DraggableCard(
        ...
        DraggableCard(
          ...
          isDraggable: widget.overlayOrNot,
...
class _DraggableCardState extends State<DraggableCard>
  ...
  Widget build(BuildContext context) {
    return new AnchoredOverlay(
      showOverlay: widget.isDraggable,
      child: new Center(),
...
class ProfileCard extends StatefulWidget {
  final User user;
  /// to show tinderCard with profile
